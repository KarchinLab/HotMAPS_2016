...
def get_group_pvals(mupit_groups, gene_ind, transcript_ind,
    ...
    # as values
    hspot_pvals = {}
    # same dictionary except with z-score
    hspot_zscores = {}
    ...
    for hspot_data in hotspot_output:
        ...
        if not curr_key in hspot_pvals:
            ...
            hspot_zscores[curr_key] = [float(hspot_data[6])]
        else:
            hspot_pvals[curr_key].append(float(hspot_data[5]))
            hspot_zscores[curr_key].append(float(hspot_data[6]))
    ...
    for j, line in enumerate(mupit_groups):
        ...
        for zscore in hspot_zscores[pdb_chain_residue]:
            grouped_zscores.append(zscore)
...
def main(opts):
    ...
    header = ["HUGO Symbol", "Tumor Type",
              ...
              "Min p-value", 'Max density z-score', 'q-value']
    ...
    for m_file in os.listdir(mupit_dir):
        ...
        grouped_p_vals = get_group_pvals(mupit_annotations, gene_ind,
                                         ...
                                         ttype, myfunc)
        # add the q-value
        tmp_pvals = [g[-2] for g in grouped_p_vals]
